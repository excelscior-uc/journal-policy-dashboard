...
// --- Field-level journal/policy counts (membership = the `All_Fields` list) ---
// Derived from the per-field JSON so grid counts match what each field page renders.

export function buildFieldCounts(rows, fieldJsons) {
  const out = {}
  for (const fj of fieldJsons) {
    out[fj.slug] = {
      totalJournals: fj.journals.length,
      withPolicy: fj.journals.filter(j => j.hasPolicy).length,
    }
  }
  const all = new Set()
  const pol = new Set()
  for (const r of rows) {
    ...
    if (!abbrev) continue
    all.add(abbrev)
    if (num(r.policy) === 1) pol.add(abbrev)
  }
  out['all-fields'] = { totalJournals: all.size, withPolicy: pol.size }
  return out
}

// --- Site-wide headline stats (all derived from the CSV) ---

export function buildSiteStats(rows, fieldJsons) {
  const all = new Set()
  const pol = new Set()
  ...
  for (const r of rows) {
    ...
    const y = num(r.year)
    if (y != null) { if (y < yearMin) yearMin = y; if (y > yearMax) yearMax = y }
  }
  const chartCount = fieldJsons.reduce((s, f) => s + (f.journals?.length ?? 0), 0)
  return {
    totalJournals: all.size,
    withPolicy: pol.size,
    fieldCount: Object.keys(FIELD_DISPLAY_MAP).length,
    chartCount,
    yearMin,
    yearMax,
...
function writeSiteStats(stats) {
  const p = join(__dirname, '../src/data/siteStats.js')
  const body = `// AUTO-GENERATED by scripts/extract-data.js from the source CSV. Do not edit by hand.
export const SITE_STATS = ${JSON.stringify(stats, null, 2)}
`
...
if (process.argv[1] && process.argv[1].endsWith('extract-data.js')) {
  ...
  writeFieldsCounts(buildFieldCounts(rows, fieldJsons))
  console.log('OK src/data/fields.js counts')

  writeSiteStats(buildSiteStats(rows, fieldJsons))
  console.log('OK src/data/siteStats.js')
}
